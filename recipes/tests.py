...
class RecipeAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.ingredient = Ingredient.objects.create(
            name="Сахар", measurement_unit="граммы"
        )
        ...
        self.recipe.ingredients.add(self.ingredient, through_defaults={"amount": 100})
    ...
    def test_remove_from_favorite(self):
        ...
        response = self.client.delete(f"/api/recipes/{self.recipe.id}/unfavorite/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(
            Favorite.objects.filter(user=self.user, recipe=self.recipe).exists()
        )
